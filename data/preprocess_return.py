...
def return_matrix(folder_path, delta = 1e-07):
    flag = 0
    # 遍历文件夹下所有的文件
...
# 获得最终设计矩阵
matrix1 = pd.read_csv('./code/data/return2020.csv', low_memory=False)
matrix2 = pd.read_csv('./code/data/return2021.csv', low_memory=False)
matrix3 = pd.read_csv('./code/data/return2022.csv', low_memory=False)
design_M = pd.concat([matrix1, matrix2,matrix3], ignore_index=True, sort=False)
design_M = design_M.drop(columns='Unnamed: 0')
design_M.to_csv('./code/data/return_matrix.csv')